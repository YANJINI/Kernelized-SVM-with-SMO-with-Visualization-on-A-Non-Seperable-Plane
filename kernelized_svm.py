...
class SMO_kernel_SVM:
    def __init__(self, kernel="linear", C=10000.0, max_iter=1000, degree=3, gamma=1):
        self.kernel = {
            ...
            "rbf": lambda x, y: np.exp(
                -gamma * np.sum((y - x[:, np.newaxis]) ** 2, axis=-1)
            ),
    ...
    def _SMO_fit(self):
        self.lambdas = np.zeros_like(
            self.y, dtype=float
        )  # lambda vector whose entries are all zeros to statisfy constraints
        self.K = (
            self.kernel(self.X, self.X) * self.y[:, np.newaxis] * self.y
        )  # n by n kernel matrix, with elements of k(x_i, x_j)y_iy_j

        for _ in range(self.max_iter):
            for idxM in range(
                len(self.lambdas)
            ):  # optimization of random pair (lam_L, lam_M)
                idxL = np.random.randint(
                    0, len(self.lambdas)
                )  # choose lambda_L randomly
                Q = self.K[
                    [[idxM, idxM], [idxL, idxL]],   #    [MM ML]
                    [[idxM, idxL],             ]    #    [LM LL] matrix
                ]
                v0 = self.lambdas[[idxM, idxL]]  # v0 = [lam_M, lam_L]^T
                k0 = 1 - np.sum(
                    self.lambdas * self.K[[idxM, idxL]], axis=1
                )  # 1-sum(lam_j*k(x_M, x_j), 1-sum(lam_i*k(x_L, x_i)
                u = np.array([-self.y[idxL], self.y[idxM]])
                t_star = np.dot(k0, u) / (
                    np.dot(np.dot(Q, u), u) + 1e-15
                )  # t* = (k0^Tu)/(u^TQu)
                self.lambdas[[idxM, idxL]] = v0 + u * self._restrict_to_square(
                    t_star, v0, u
                )  # update new lams
            ...
            self.b = np.sum(
                ...
                idx
            )  # sum_k(y_k*(1-sum_i(lam_i*K_ki)) / k

    def _SMO_decision_function(self, X):
        return (
            np.sum(self.kernel(X, self.X) * self.y * self.lambdas, axis=1) + self.b
        )  # classifier

    def _restrict_to_square(
        self, t_star, v0, u
    ):  # restrict given t_star for lam_M(t_star), lam_L(t_star) to be [0, C]
        restricted_t_star_first_ineq = (np.clip(v0 + t_star * u, 0, self.C) - v0)[
            0
        ] / u[0]
        restricted_t_star_second_ineq = (
            np.clip(v0 + restricted_t_star_first_ineq * u, 0, self.C) - v0
        )[1] / u[1]
    ...
    def _background_figure(self):
        if self.enter_status == "class1":
            ...
            self.ax.set_title(
                "click to plot from class 1. Please press enter when finished."
            )
        elif self.enter_status == "class2":
            self.ax.set_title(
                "click to plot from class 2. please enter to start kernelized SVM with SMO."
            )
            ...
            xx, yy = np.meshgrid(
                np.linspace(*[-5, 5], num=700), np.linspace(*[-5, 5], num=700)
            )
            z_model = self._SMO_decision_function(
